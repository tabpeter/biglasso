...
fit.ssr.slores <- biglasso(X.bm, y, family = 'binomial', eps = eps, screen = 'SSR-Slores', lambda.min = 0)
fit.batch <- biglasso(X.bm, y, family = 'binomial', eps = eps, screen = 'SSR-Slores-Batch', lambda.min = 0)

test_that("Test against MLE: ",{
  expect_equal(as.numeric(beta), as.numeric(fit.ssr$beta[, 100]), tolerance = tolerance)
  expect_equal(as.numeric(fit.ssr$beta[, 100]), as.numeric(fit.ssr.slores$beta[, 100]), tolerance = tolerance)
  expect_equal(as.numeric(fit.ssr$beta[, 100]), as.numeric(fit.ssr.mm$beta[, 100]), tolerance = tolerance)
  expect_equal(as.numeric(fit.ssr$beta[, 100]), as.numeric(fit.batch$beta[, 100]), tolerance = tolerance)
})
...
test_that("Test against glmnet: ",{
  ...
  expect_equal(as.numeric(fit.glm$beta), as.numeric(fit.ssr.slores$beta[-1, ]), tolerance = tolerance)
  expect_equal(as.numeric(fit.glm$beta), as.numeric(fit.batch$beta[-1, ]), tolerance = tolerance)
})

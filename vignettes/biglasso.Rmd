...
```{r}
## The data has 1000 observations and 100,000 features.
## 10 of the features has non-zero coefficients
if(!file.exists('BigX.csv')) {
  X <- matrix(rnorm(1000 * 100000), 1000, 100000)
  beta <- c(-5:5)
  y <- as.numeric(X[,1:11] %*% beta)
  write.csv(X, "BigX.csv", row.names = F)
  write.csv(y, "y.csv", row.names = F)
  ## Pretend the data is stored in the ~1G .csv file and is too large to fit into memory
...
* biglasso R manual: https://cran.rstudio.com/package=biglasso/biglasso.pdf

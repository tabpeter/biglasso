...
SEXP cleanupB(double *s, double *w, double *a, double *r, int *e1, int *e2, double *z, 
              double *eta, SEXP beta0, SEXP beta, SEXP Dev, SEXP iter) {
  Free(s);
  Free(w);
  Free(a);
  Free(r);
...
//  double gamma = REAL(gamma_)[0];
  ...
  // temp result: sum of w * r
  double sumWResid = 0.0;
  
  for (int j=0; j<p; j++) {
    z[j] = crossprod_bm(X_, s, row_idx, center[j], scale[j], n, j)/n;
  }
      ...
      while (INTEGER(iter)[l] < max_iter) {
      	while (INTEGER(iter)[l] < max_iter) {
      	  ...
      	  for (int j=0; j<p; j++) {
      	    if (e1[j]) {
      	      // Calculate u, v
      	      xwr = wcrossprod_resid(X_, r, sumWResid, row_idx, center[j], scale[j], w, n, j);
      	      xwx = wsqsum_bm(X_, w, row_idx, center[j], scale[j], n, j);
      	      u = xwr/n + (xwx/n)*a[j];
      	      v = xwx/n;
      	      ...
      	      if (shift !=0) {
      	        ...
      	        for (int i=0;i<n;i++) {
      	          si = shift*get_elem_bm(X_, center[j], scale[j], row_idx[i], j);
      	          r[i] -= si;
      	          eta[i] += si;

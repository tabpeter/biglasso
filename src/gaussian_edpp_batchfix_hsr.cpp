...
RcppExport SEXP cdfit_gaussian_edpp_batchfix_hsr(SEXP X_, SEXP y_, SEXP row_idx_, SEXP lambda_, SEXP nlambda_,
					      SEXP lam_scale_, SEXP lambda_min_, SEXP alpha_, SEXP user_,
					      SEXP eps_, SEXP max_iter_, SEXP multiplier_, SEXP dfmax_,
					      SEXP ncore_, SEXP bsize_, SEXP verbose_) {
  ...
  for (l = lstart; l < L; l++) {
	...
	Free(ever_active); Free(r); Free(a); Free(discard_beta); Free(lhs2); Free(Xty); Free(Xtr); Free(yhat); Free(discard_old); Free(strong_set);
      ...
      if(l % bsize == 0) { // Recalculate SEDPP after every bsize lambdas.
	if(verbose) {
          // output time
          char buff[100];
          time_t now = time (0);
